...
当前版本：`v4.7.10`。MoviePilot 新版 / PostgreSQL 下的订阅媒体类型统一转换为 MoviePilot 枚举；115 分享缺少大小、做种、免费或发布时间时只跳过这些不可验证规则字段，身份、季号、字幕与清晰度门槛保持严格。识别不可用时只恢复订阅，绝不降级为仅按标题自动转存。通知标题显示年份和季号，例如 `TG115 搜索完成：片名（2023）S03`。周期搜索仍按独立订阅行处理 S00/S02/S03，且所有来源结束后合并通知一次。
